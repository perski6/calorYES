        ...
        Swal.fire({
            ...
            preConfirm: (ammount) =>{
                ...
                dispatch(addBalance({
                    ammount:ammount,
                    name:nameJson,
                    calories:caloriesJson*ammount/100,
                }))
        ...
        <div className="food-page">
            {foods.map((food) => (
                <div className="food-listing" key={food._id}>
                    <h2>Name:{food.name} Calories:{food.calories_per_100}<button onClick={()=>handleAdd(food.name,food.calories_per_100)}>Add</button></h2>
                </div>
            ))}
            </div>
